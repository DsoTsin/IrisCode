...
bool extract_pe_dependencies(string const& exe,
  ...
  for (const auto& sd : dll_search_paths) {
    search_dirs.push_back(sd);
  }
  ...
  while (!pequeue.empty()) {
    ...
    if (!path::is_absolute(pending_pe_abspth)) {
      ...
      // std::transform(pending_pe_abspth.begin(), pending_pe_abspth.end(),
      // pending_pe_abspth.begin(),
      //                ::tolower);
    }
    string copy = pending_pe_abspth;
    std::transform(copy.begin(), copy.end(), copy.begin(), ::tolower);
    if (!copy.empty() /* && !copy.startswith("c:/windows/system32/")*/) {
      pe_file pfile{pending_pe, ""};
      if (is_sys_dll(pending_pe) || walked_pes.find(pfile) != walked_pes.end()) {
        continue;
      }
      pfile.abs_path = pending_pe_abspth;
      if (path::is_absolute(pfile.name)) {
        pfile.name = move(path::file_name(pending_pe_abspth));
      }
      walked_pes.insert(move(pfile));
...
// IMAGE_NT_HEADERS64, IMAGE_NT_HEADERS32.
template <class ImageNtHeader>
inline pe_file_list readImportSections(const ImageNtHeader* ntHeaders,
                                       const void* base,
...
template <class ImageNtHeader>
inline void determineDebugAndDependentLibs(const ImageNtHeader* nth,
                                           const void* fileMemory,
                                           pe_file_list& dependentLibraries,
                                           bool isMinGW,
                                           bool* isDebugIn,
                                           std::string* errorMessage) {
...
bool read_pe_executable(string const& exe, pe_file_list& out_pes, string* err_msg) {
  ...
  do {
    ...
    hFile = CreateFileA(reinterpret_cast<const CHAR*>(exe.c_str()), GENERIC_READ, FILE_SHARE_READ,
    ...
    //    *wordSizeIn = wordSize;
    if (wordSize == 32) {
      determineDebugAndDependentLibs<IMAGE_NT_HEADERS32>(
          reinterpret_cast<const IMAGE_NT_HEADERS32*>(ntHeaders), fileMemory, out_pes, false,
          &isDebugIn, err_msg);
    } else {
      determineDebugAndDependentLibs<IMAGE_NT_HEADERS64>(
          reinterpret_cast<const IMAGE_NT_HEADERS64*>(ntHeaders), fileMemory, out_pes, false,
          &isDebugIn, err_msg);
    }
...
static std::unordered_set<std::string> blacklist
    = {"kernel32.dll", "kernelbase.dll",    "ws2_32.dll",
       "user32.dll",   "advapi32.dll",      "ntdll.dll",
       "ole32.dll",    "shell32.dll",       "shlwapi.dll",
       "dbghelp.dll",  "winmm.dll",         "gdi32.dll",
       "comdlg32.dll", "oleaut32.dll",      "netapi32.dll",
       "iphlpapi.dll", "winhttp.dll",       "version.dll",
       "dwmapi.dll",   "wininet.dll",       "setupapi.dll",
       "imm32.dll",    "psapi.dll",         "rpcrt4.dll",
       "userenv.dll",  "ncrypt.dll",        "pdh.dll",
       "uxtheme.dll",  "msasn1.dll",        "d2d1.dll",
       "d3d11.dll",    "d3d11on12.dll",     "dwrite.dll",
       "sspicli.dll",  "dnsapi.dll",        "bcryptprimitives.dll",
       "d3d12.dll",    "nsi.dll",           "bcrypt.dll",
       "wldap32.dll",  "fltlib.dll",        "ntasn1.dll",
       "dpapi.dll",    "dsparse.dll",       "crypt32.dll",
       "cryptsp.dll",  "bcp47langs.dll",    "cfgmgr32.dll",
       "cabinet.dll",  "coremessaging.dll", "d3dscache.dll",
       "glu32.dll",    "oleacc.dll",        "opengl32.dll",
       "powrprof.dll", "propsys.dll",       "mpr.dll",
       "samlib.dll",   "dxgi.dll",          "wer.dll",
       "faultrep.dll", "xmllite.dll",       "uiautomationcore.dll",
       "win32u.dll",   "combase.dll",       "umpdc.dll",
       "wmiclnt.dll",  "cryptbase.dll",     "api-ms-*.dll",
       "ext-ms-*.dll"};

bool is_sys_dll(std::string const& dll) {
  auto copy = dll;
  std::transform(copy.begin(), copy.end(), copy.begin(), ::tolower);
  return blacklist.find(copy) != blacklist.end();
}
