...
{
    struct bom_context_memory memory = bom_context_memory_file(file_name, false, 0);
    if (memory.data)
        return bom_alloc_load(memory);
    else
        return NULL;
}
...
{
    int index = bom_variable_get(context, name);
    if (index != -1)
    {
        return bom_index_get(context, index, data_len);
    }
    else
    {
        return NULL;
