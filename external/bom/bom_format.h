...
IB_PACKED_START struct bom_header {
...
IB_PACKED_START struct bom_index {
  uint32_t address;
  uint32_t length;
} IB_PACKED_END;

IB_PACKED_START struct bom_index_header {
  uint32_t count; // FIXME: What is this? It is not the length of the array...
  struct bom_index index[0];
} IB_PACKED_END;

IB_PACKED_START struct bom_variable {
  uint32_t index;
  uint8_t length;
  char name[0]; // length
} IB_PACKED_END;

IB_PACKED_START struct bom_variables {
  uint32_t count; // Number of entries that follow
  // Followed by bom_variable entries
} IB_PACKED_END;

IB_PACKED_START struct bom_tree { // 21 bytes
  char magic[4]; // = "tree"
  uint32_t version;
...
IB_PACKED_START struct bom_tree_entry_indexes {
  uint32_t value_index;
  uint32_t key_index;
} IB_PACKED_END;

IB_PACKED_START struct bom_tree_entry {
