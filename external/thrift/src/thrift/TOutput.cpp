...
void TOutput::perror(const char* message, int errno_copy, int line, const char* source)
{
  std::ostringstream ess;
  ess << message << ": " << strerror_s(errno_copy) << " @" << source << ":" << line;
  std::string out = ess.str();
  f_(out.c_str());
}
...
std::string TOutput::strerror_s(int errno_copy) {
  ...
  BOOL fOk = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS
                           ...
                           NULL, errno_copy, systemLocale, (PTSTR)&hlocal, 0, NULL);
  std::string msg;
  if (fOk && (hlocal != NULL)) {
    msg = (PCTSTR) LocalLock(hlocal);
    LocalFree(hlocal);
  }
  return "errno = " + to_string(errno_copy) + ", msg = " + msg;
#else
