...
void TServerSocket::listen() {
  ...
  if (-1 == THRIFT_SOCKETPAIR(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() socketpair() interrupt", THRIFT_GET_SOCKET_ERROR);
    interruptSockWriter_ = THRIFT_INVALID_SOCKET;
    interruptSockReader_ = THRIFT_INVALID_SOCKET;
  } else {
  ...
  if (-1 == THRIFT_SOCKETPAIR(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() socketpair() childInterrupt",
  ...
  if (serverSocket_ == THRIFT_INVALID_SOCKET) {
    ...
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() socket() ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
...
// want to force servers to be an admin.
#ifndef _WIN32
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() THRIFT_NO_SOCKET_CACHING ",
                        errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (tcpSendBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_,
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() SO_SNDBUF ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (tcpRecvBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_,
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() SO_RCVBUF ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
...
// Defer accept
  ...
  if (path_.empty()) {
    if (-1 == setsockopt(serverSocket_, IPPROTO_TCP, TCP_DEFER_ACCEPT, &one, sizeof(one))) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() TCP_DEFER_ACCEPT ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (res->ai_family == AF_INET6 && path_.empty()) {
    ...
    if (-1 == setsockopt(serverSocket_,
                         ...
                         cast_sockopt(&zero),
                         sizeof(zero))) {
      THRIFT_ERROR_WITH_LINE("TServerSocket::listen() IPV6_V6ONLY ", THRIFT_GET_SOCKET_ERROR);
    }
  }
  ...
  if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_LINGER, cast_sockopt(&ling), sizeof(ling))) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() SO_LINGER ", errno_copy);
    close();
  ...
  if (reusePort_) {
    int value = 1;
    if (-1
        == setsockopt(serverSocket_, SOL_SOCKET, SO_REUSEADDR, cast_sockopt(&value),
                      sizeof(value))) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() SO_REUSEADDR ", errno_copy);
      close();
  ...
  if (path_.empty()) {
    ...
    if (-1
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TServerSocket::listen() setsockopt() TCP_NODELAY ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (flags == -1) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() THRIFT_FCNTL() THRIFT_F_GETFL ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (-1 == THRIFT_FCNTL(serverSocket_, THRIFT_F_SETFL, flags | THRIFT_O_NONBLOCK)) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() THRIFT_FCNTL() THRIFT_O_NONBLOCK ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (!path_.empty()) {
    ...
    if (len > sizeof(((sockaddr_un*)nullptr)->sun_path)) {
      ...
      THRIFT_ERROR_WITH_LINE("TSocket::listen() Unix Domain socket path too long", errno_copy);
      throw TTransportException(TTransportException::NOT_OPEN,
    ...
    if (port_ == 0 && retries <= retryLimit_) {
      ...
      if (::getsockname(serverSocket_, reinterpret_cast<struct sockaddr*>(&sa), &len) < 0) {
        errno_copy = THRIFT_GET_SOCKET_ERROR;
        THRIFT_ERROR_WITH_LINE("TServerSocket::getPort() getsockname() ", errno_copy);
      } else {
  ...
  if (-1 == ::listen(serverSocket_, acceptBacklog_)) {
    errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TServerSocket::listen() listen() ", errno_copy);
    close();
...
shared_ptr<TTransport> TServerSocket::acceptImpl() {
  ...
  while (true) {
    ...
    if (ret < 0) {
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TServerSocket::acceptImpl() THRIFT_POLL() ", errno_copy);
      ...
      if (interruptSockReader_ != THRIFT_INVALID_SOCKET && (fds[1].revents & THRIFT_POLLIN)) {
        ...
        if (-1 == recv(interruptSockReader_, cast_sockopt(&buf), sizeof(int8_t), 0)) {
          THRIFT_ERROR_WITH_LINE("TServerSocket::acceptImpl() recv() interrupt ",
                              THRIFT_GET_SOCKET_ERROR);
        }
  ...
  if (clientSocket == THRIFT_INVALID_SOCKET) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TServerSocket::acceptImpl() ::accept() ", errno_copy);
  ...
  if (flags == -1) {
    ...
    THRIFT_ERROR_WITH_LINE("TServerSocket::acceptImpl() THRIFT_FCNTL() THRIFT_F_GETFL ", errno_copy);
    throw TTransportException(TTransportException::UNKNOWN,
...
void TServerSocket::notify(THRIFT_SOCKET notifySocket) {
  if (notifySocket != THRIFT_INVALID_SOCKET) {
    ...
    if (-1 == send(notifySocket, cast_sockopt(&byte), sizeof(int8_t), 0)) {
      THRIFT_ERROR_WITH_LINE("TServerSocket::notify() send() ", THRIFT_GET_SOCKET_ERROR);
    }
  }
}
