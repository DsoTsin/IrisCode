...
try_again:
  ...
  if (r == -1) {
    ...
    THRIFT_ERROR_WITH_LINE("TSocket::hasPendingDataToRead() THRIFT_IOCTL_SOCKET() " + getSocketInfo(), errno_copy);
  ...
  if (interruptListener_) {
    for (int retries = 0;;) {
      ...
      if (ret < 0) {
        ...
        THRIFT_ERROR_WITH_LINE("TSocket::peek() THRIFT_POLL() ", errno_copy);
        throw TTransportException(TTransportException::UNKNOWN, "Unknown", errno_copy);
      } else if (ret > 0) {
...
#endif
    THRIFT_ERROR_WITH_LINE("TSocket::peek() recv() " + getSocketInfo(), errno_copy);
...
void TSocket::openConnection(struct addrinfo* res) {
  ...
  if (socket_ == THRIFT_INVALID_SOCKET) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TSocket::open() socket() " + getSocketInfo(), errno_copy);
  ...
  if (keepAlive_) {
    setKeepAlive(keepAlive_, keepAliveInterval_);
  }
...
// Uses a low min RTO if asked to.
  ...
  if (connTimeout_ > 0) {
    if (-1 == THRIFT_FCNTL(socket_, THRIFT_F_SETFL, flags | THRIFT_O_NONBLOCK)) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TSocket::open() THRIFT_FCNTL() " + getSocketInfo(), errno_copy);
  ...
  if (!path_.empty()) {
    ...
    if (len > sizeof(((sockaddr_un*)nullptr)->sun_path)) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TSocket::open() Unix Domain socket path too long", errno_copy);
...
#else
      THRIFT_ERROR_WITH_LINE("TSocket::open() Abstract Namespace Domain sockets only supported on linux: ", -99);
...
#else
    THRIFT_ERROR_WITH_LINE("TSocket::open() Unix Domain socket path not supported on windows", -99);
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if ((THRIFT_GET_SOCKET_ERROR != THRIFT_EINPROGRESS)
    ...
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TSocket::open() connect() " + getSocketInfo(), errno_copy);
  ...
  if (ret > 0) {
    ...
    if (ret2 == -1) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TSocket::open() getsockopt() " + getSocketInfo(), errno_copy);
    ...
    THRIFT_ERROR_WITH_LINE("TSocket::open() error on socket (after THRIFT_POLL) " + getSocketInfo(),
                        val);
    ...
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TSocket::open() THRIFT_POLL() " + getSocketInfo(), errno_copy);
...
try_again:
  ...
  if (interruptListener_) {
    ...
    if (ret < 0) {
      ...
      THRIFT_ERROR_WITH_LINE("TSocket::read() THRIFT_POLL() ", errno_copy);
      throw TTransportException(TTransportException::UNKNOWN, "Unknown", errno_copy);
    } else if (ret > 0) {
      // Check the interruptListener
      if (fds[1].revents & THRIFT_POLLIN) {
  ...
  if (got < 0) {
    ...
    THRIFT_ERROR_WITH_LINE("TSocket::read() recv() " + getSocketInfo(), errno_copy);
...
uint32_t TSocket::write_partial(const uint8_t* buf, uint32_t len) {
  ...
  if (b < 0) {
    ...
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    THRIFT_ERROR_WITH_LINE("TSocket::write_partial() send() " + getSocketInfo(), errno_copy);
...
void TSocket::setLinger(bool on, int linger) {
  ...
  if (ret == -1) {
    ...
    THRIFT_ERROR_WITH_LINE("TSocket::setLinger() setsockopt() " + getSocketInfo(), errno_copy);
  }
}
...
void TSocket::setNoDelay(bool noDelay) {
  ...
  if (ret == -1) {
    int errno_copy
        = THRIFT_GET_SOCKET_ERROR; // Copy THRIFT_GET_SOCKET_ERROR because we're allocating memory.
    THRIFT_ERROR_WITH_LINE("TSocket::setNoDelay() setsockopt() " + getSocketInfo(), errno_copy);
  }
}

void TSocket::setReuseAddr(bool reuseAddr) {
  if (reuseAddr != reuseAddr_) {
    reuseAddr_ = reuseAddr;
    if (socket_ == THRIFT_INVALID_SOCKET || !path_.empty()) {
      return;
    }
    int value = reuseAddr_ ? 1 : 0;
    if (-1 == setsockopt(socket_, SOL_SOCKET, SO_REUSEADDR, cast_sockopt(&value), sizeof(value))) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TTSocket::setReuseAddr() setsockopt() SO_REUSEADDR "
                                 + getSocketInfo(),
                             errno_copy);
    }
  }
...
void setGenericTimeout(THRIFT_SOCKET s, int timeout_ms, int optname) {
  ...
  if (ret == -1) {
    int errno_copy
        = THRIFT_GET_SOCKET_ERROR; // Copy THRIFT_GET_SOCKET_ERROR because we're allocating memory.
    THRIFT_ERROR_WITH_LINE("TSocket::setGenericTimeout() setsockopt() ", errno_copy);
  }
}
...
void TSocket::setKeepAlive(bool keepAlive, int interval) {
  ...
  if (ret == -1) {
    int errno_copy
        = THRIFT_GET_SOCKET_ERROR; // Copy THRIFT_GET_SOCKET_ERROR because we're allocating memory.
    THRIFT_ERROR_WITH_LINE("TSocket::setKeepAlive() setsockopt() keepAlive " + getSocketInfo(), errno_copy);
  }

  if (interval != -1) {
    value = interval;
    ret = setsockopt(socket_, SOL_SOCKET, TCP_KEEPINTVL, const_cast_sockopt(&value), sizeof(value));
    if (ret == -1) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      THRIFT_ERROR_WITH_LINE("TSocket::setKeepAlive() setsockopt() keepAlive interval " + getSocketInfo(), errno_copy);
    }
  }
}
...
int TSocket::getLocalPort() const {
  if (local_client_port_ == 0) {
    struct sockaddr_in sin;
    socklen_t addrlen = sizeof(sin);
    if (::getsockname(socket_, (struct sockaddr*)getLocalCacheAddress(&addrlen), &addrlen) == 0) {
      local_client_port_ = ntohs(cachedLocalAddr_.ipv4.sin_port);
    }
  }
  return local_client_port_;
}
...
sockaddr* TSocket::getLocalCacheAddress(socklen_t* len) const {
  switch (cachedPeerAddr_.ipv4.sin_family) {
  case AF_INET:
    *len = sizeof(sockaddr_in);
    return (sockaddr*)&cachedLocalAddr_.ipv4;

  case AF_INET6:
    *len = sizeof(sockaddr_in6);
    return (sockaddr*)&cachedLocalAddr_.ipv6;
