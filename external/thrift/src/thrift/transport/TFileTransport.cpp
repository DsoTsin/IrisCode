...
void TFileTransport::resetOutputFile(int fd, string filename, off_t offset) {
  ...
  if (fd_ > 0) {
    ...
    if (-1 == ::THRIFT_CLOSE(fd_)) {
      ...
      THRIFT_ERROR_WITH_LINE("TFileTransport: resetOutputFile() ::close() ", errno_copy);
...
TFileTransport::~TFileTransport() {
  ...
  if (fd_ > 0) {
    if (-1 == ::THRIFT_CLOSE(fd_)) {
      THRIFT_ERROR_WITH_LINE("TFileTransport: ~TFileTransport() ::close() ", THRIFT_ERRNO);
    } else {
      // successfully closed fd
      fd_ = 0;
    }
...
void TFileTransport::writerThread() {
  ...
  if (!fd_) {
    try {
      ...
      int errno_copy = THRIFT_ERRNO;
      THRIFT_ERROR_WITH_LINE("TFileTransport: writerThread() openLogFile() ", errno_copy);
      fd_ = 0;
      hasIOError = true;
    }
  }
  ...
  if (!hasIOError) {
    try {
      ...
      if (0 == THRIFT_FTRUNCATE(fd_, offset_)) {
        ...
        int errno_copy = THRIFT_ERRNO;
        THRIFT_ERROR_WITH_LINE("TFileTransport: writerThread() truncate ", errno_copy);
        hasIOError = true;
      }
    } catch (...) {
      int errno_copy = THRIFT_ERRNO;
      THRIFT_ERROR_WITH_LINE("TFileTransport: writerThread() initialization ", errno_copy);
      hasIOError = true;
    }
  }
  ...
  while (1) {
    ...
    if (closing_) {
      ...
      if (enqueueBuffer_->isEmpty() && dequeueBuffer_->isEmpty()) {
        ...
        if (-1 == ::THRIFT_CLOSE(fd_)) {
          int errno_copy = THRIFT_ERRNO;
          THRIFT_ERROR_WITH_LINE("TFileTransport: writerThread() ::close() ", errno_copy);
        } else {
      ...
      while (nullptr != (outEvent = dequeueBuffer_->getNext())) {
        ...
        if ((outEvent->eventSize_ > 0) && (chunkSize_ != 0)) {
          ...
          if (chunk1 != chunk2) {
            ...
            if (-1 == ::write(fd_, zeros, padding)) {
              int errno_copy = THRIFT_ERRNO;
              THRIFT_ERROR_WITH_LINE("TFileTransport: writerThread() error while padding zeros ",
                                  errno_copy);
              hasIOError = true;
              continue;
            }
        ...
        if (outEvent->eventSize_ > 0) {
          if (-1 == ::THRIFT_WRITE(fd_, outEvent->eventBuff_, outEvent->eventSize_)) {
            int errno_copy = THRIFT_ERRNO;
            THRIFT_ERROR_WITH_LINE("TFileTransport: error while writing event ", errno_copy);
            hasIOError = true;
            continue;
          }
...
void TFileTransport::openLogFile() {
  ...
  if (fd_ == -1) {
    int errno_copy = THRIFT_ERRNO;
    THRIFT_ERROR_WITH_LINE("TFileTransport: openLogFile() ::open() file: " + filename_, errno_copy);
