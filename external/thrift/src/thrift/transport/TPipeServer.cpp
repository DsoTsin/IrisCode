...
public:
  TAnonPipeServer() {
    ...
    // blocking call.
    if (!createAnonPipe()) {
      THRIFT_ERROR_WITH_LINE("TPipeServer Create(Anon)Pipe failed, GLE=", GetLastError());
...
shared_ptr<TTransport> TAnonPipeServer::acceptImpl() {
  ...
  if (!fSuccess && GetLastError() != ERROR_MORE_DATA) {
    THRIFT_ERROR_WITH_LINE("TPipeServer unable to initiate pipe comms, GLE=", GetLastError());
    throw TTransportException(TTransportException::NOT_OPEN,
...
void TNamedPipeServer::initiateNamedConnect(const TAutoCrit &lockProof) {
  ...
  if (!createNamedPipe(lockProof)) {
    THRIFT_ERROR_WITH_LINE("TPipeServer CreateNamedPipe failed, GLE=", GetLastError());
  ...
  default:
    THRIFT_ERROR_WITH_LINE("TPipeServer ConnectNamedPipe failed, GLE=", dwErr);
    throw TTransportException(TTransportException::NOT_OPEN,
...
shared_ptr<TTransport> TNamedPipeServer::acceptImpl() {
  ...
  THRIFT_ERROR_WITH_LINE("TPipeServer ConnectNamedPipe GLE=", gle);
...
bool TNamedPipeServer::createNamedPipe(const TAutoCrit & /*lockProof*/) {
  ...
  if (!InitializeSecurityDescriptor(sd, SECURITY_DESCRIPTOR_REVISION)) {
    ...
    LocalFree(sd);
    LocalFree(acl);
    THRIFT_ERROR_WITH_LINE("TPipeServer::InitializeSecurityDescriptor() GLE=", lastError);
  ...
  if (!SetSecurityDescriptorDacl(sd, TRUE, acl, FALSE)) {
    ...
    LocalFree(sd);
    LocalFree(acl);
    THRIFT_ERROR_WITH_LINE("TPipeServer::SetSecurityDescriptorDacl() GLE=", lastError);
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (hPipe.h == INVALID_HANDLE_VALUE) {
    Pipe_.reset();
    THRIFT_ERROR_WITH_LINE("TPipeServer::TCreateNamedPipe() GLE=", lastError);
    throw TTransportException(TTransportException::NOT_OPEN,
...
bool TAnonPipeServer::createAnonPipe() {
  ...
  if (!InitializeSecurityDescriptor(&sd, SECURITY_DESCRIPTOR_REVISION))
  {
    THRIFT_ERROR_WITH_LINE("TPipeServer InitializeSecurityDescriptor (anon) failed, GLE=", GetLastError());
    return false;
  }
  if (!SetSecurityDescriptorDacl(&sd, true, NULL, false))
  {
    THRIFT_ERROR_WITH_LINE("TPipeServer SetSecurityDescriptorDacl (anon) failed, GLE=",
                        GetLastError());
    return false;
  }
  sa.lpSecurityDescriptor = &sd;
  ...
  {
    THRIFT_ERROR_WITH_LINE("TPipeServer CreatePipe (anon) failed, GLE=", GetLastError());
    return false;
  }
