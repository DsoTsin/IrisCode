...
extern "C" {
HANDLE(WINAPI* thrift_CreateFileA)
(LPCSTR, DWORD, DWORD, LPSECURITY_ATTRIBUTES, DWORD, DWORD, HANDLE) = imp_CreateFileA;
BOOL(WINAPI* thrift_WaitNamedPipeA)(LPCSTR, DWORD) = WaitNamedPipeA;
BOOL(WINAPI* thrift_ReadFile)
(HANDLE, LPVOID, DWORD, LPDWORD, LPOVERLAPPED) = imp_ReadFile;
BOOL(WINAPI* thrift_WriteFile)
(HANDLE, LPCVOID, DWORD, LPDWORD, LPOVERLAPPED) = imp_WriteFile;
#define CreateFileA thrift_CreateFileA
#define WaitNamedPipeA thrift_WaitNamedPipeA
#define ReadFile thrift_ReadFile
#define WriteFile thrift_WriteFile
}
#endif
...
void TWaitableNamedPipeImpl::beginAsyncRead(uint8_t* buf, uint32_t len) {
  ...
  if (readOverlap_.success == FALSE && readOverlap_.last_error != ERROR_IO_PENDING) {
    THRIFT_ERROR_WITH_LINE("TPipe ::ReadFile errored GLE=", readOverlap_.last_error);
...
void pseudo_sync_write(HANDLE pipe, HANDLE event, const uint8_t* buf, uint32_t len) {
  ...
  while (written < len) {
    ...
    if (result == FALSE && ::GetLastError() != ERROR_IO_PENDING) {
      THRIFT_ERROR_WITH_LINE("TPipe ::WriteFile errored GLE=", ::GetLastError());
    ...
    if (!result) {
      THRIFT_ERROR_WITH_LINE("TPipe ::GetOverlappedResult errored GLE=", ::GetLastError());
...
uint32_t pseudo_sync_read(HANDLE pipe, HANDLE event, uint8_t* buf, uint32_t len) {
  ...
  if (result == FALSE && ::GetLastError() != ERROR_IO_PENDING) {
    THRIFT_ERROR_WITH_LINE("TPipe ::ReadFile errored GLE=", ::GetLastError());
...
TPipe::TPipe(HANDLE Pipe) : TimeoutSeconds_(3), isAnonymous_(false) {
...
void TPipe::open() {
  ...
  do {
    ...
    hPipe.reset(CreateFileA(pipename_.c_str(), GENERIC_READ | GENERIC_WRITE,
                            0,             // no sharing
    ...
    if (ret != ERROR_PIPE_BUSY) {
      THRIFT_ERROR_WITH_LINE("TPipe::open ::CreateFile errored GLE=", ret);
  ...
  if (hPipe.h == INVALID_HANDLE_VALUE) {
    THRIFT_ERROR_WITH_LINE("TPipe::open ::CreateFile errored GLE=", ::GetLastError());
    throw TTransportException(TTransportException::NOT_OPEN, "Unable to open pipe");
  }
