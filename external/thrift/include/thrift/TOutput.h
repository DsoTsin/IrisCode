...
public:
  ...
  void perror(const char* message, int errno_copy, int line, const char* source);
  
  inline void perror(const std::string& message, int errno_copy) {
    perror(message.c_str(), errno_copy);
  }

  inline void perror(const std::string& message, int errno_copy, int line, const char* source) {
    perror(message.c_str(), errno_copy, line, source);
  }
...
#define THRIFT_ERROR_WITH_LINE(msg, err_code) \
  GlobalOutput.perror((msg), (err_code), __LINE__, __FILE__)

#define THRIFT_PRINT(msg, ...) \
  GlobalOutput.printf(msg, __VA_ARGS__)
