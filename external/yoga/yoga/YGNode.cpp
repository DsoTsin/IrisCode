...
YGNodeRef YGNodeNewWithConfig(const YGConfigConstRef config) {
  auto* node = new iris::ui::Node{resolveRef(config)};
  iris::ui::assertFatal(
...
YGNodeRef YGNodeClone(YGNodeConstRef oldNodeRef) {
  ...
  const auto node = new iris::ui::Node(*oldNode);
...
void YGNodeRemoveAllChildren(const YGNodeRef ownerRef) {
  ...
  if (firstChild->getOwner() == owner) {
    ...
    // set is unique.
    for (size_t i = 0; i < childCount; i++) {
      iris::ui::Node* oldChild = owner->getChild(i);
...
void YGNodeSetChildren(
    ...
    const size_t count) {
  auto owner = resolveRef(ownerRef);
  auto children = reinterpret_cast<iris::ui::Node* const*>(childrenRefs);

  if (owner == nullptr) {
    return;
  }

  const std::vector<iris::ui::Node*> childrenVector = {children, children + count};
  if (childrenVector.empty()) {
    ...
    for (iris::ui::Node* child : childrenVector) {
      child->setOwner(owner);
    }
    owner->markDirtyAndPropagate();
  }
...
bool YGNodeCanUseCachedMeasurement(
    ...
    float marginColumn,
    YGConfigRef config) {
  return iris::ui::canUseCachedMeasurement(
