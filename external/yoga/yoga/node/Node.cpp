...
Node::Node(const iris::ui::Config* config) : config_{config} {
  iris::ui::assertFatal(
...
YGSize Node::measure(
  ...
  const auto size = measureFunc_(this, availableWidth, unscopedEnum(widthMode), availableHeight,
                                 unscopedEnum(heightMode));

  if (iris::ui::isUndefined(size.height) || size.height < 0 ||
      iris::ui::isUndefined(size.width) || size.width < 0) {
    iris::ui::log(
        this,
        LogLevel::Warn,
...
bool Node::isLayoutDimensionDefined(const FlexDirection axis) {
  ...
  return iris::ui::isDefined(value) && value >= 0.0f;
}
...
void Node::setConfig(iris::ui::Config* config) {
  iris::ui::assertFatal(
      config != nullptr, "Attempting to set a null config on a Node");
  iris::ui::assertFatalWithConfig(
      config,
      config->useWebDefaults() == config_->useWebDefaults(),
      "UseWebDefaults may not be changed after constructing a Node");

  if (iris::ui::configUpdateInvalidatesLayout(*config_, *config)) {
...
void Node::setPosition(
  ...
  const FlexDirection mainAxis =
      iris::ui::resolveDirection(style_.flexDirection(), directionRespectingRoot);
  const FlexDirection crossAxis =
      iris::ui::resolveCrossDirection(mainAxis, directionRespectingRoot);
...
void Node::processDimensions() {
  for (auto dim : {Dimension::Width, Dimension::Height}) {
    if (style_.maxDimension(dim).isDefined() &&
      ...
      processedDimensions_[iris::ui::to_underlying(dim)] = style_.maxDimension(dim);
    } else {
      processedDimensions_[iris::ui::to_underlying(dim)] = style_.dimension(dim);
    }
  }
}
