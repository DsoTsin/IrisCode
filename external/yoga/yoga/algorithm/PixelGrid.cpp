...
float roundValueToPixelGrid(
  ...
  if (iris::ui::inexactEquals(fractial, 0)) {
    // First we check if the value is already rounded
    scaledValue = scaledValue - fractial;
  } else if (iris::ui::inexactEquals(fractial, 1.0)) {
    ...
    scaledValue = scaledValue - fractial +
        (!std::isnan(fractial) &&
                 (fractial > 0.5 || iris::ui::inexactEquals(fractial, 0.5))
             ? 1.0
             : 0.0);
  }
...
void roundLayoutResultsToPixelGrid(
  ...
  if (pointScaleFactor != 0.0) {
    ...
    const bool hasFractionalWidth =
        !iris::ui::inexactEquals(round(scaledNodeWith), scaledNodeWith);

    const double scaledNodeHeight = nodeHeight * pointScaleFactor;
    const bool hasFractionalHeight =
        !iris::ui::inexactEquals(round(scaledNodeHeight), scaledNodeHeight);
  ...
  for (iris::ui::Node* child : node->getChildren()) {
