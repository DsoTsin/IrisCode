...
static void computeFlexBasisForChild(
  ...
  if (resolvedFlexBasis.isDefined() && iris::ui::isDefined(mainAxisSize)) {
    if (child->getLayout().computedFlexBasis.isUndefined() ||
      ...
      child->setLayoutComputedFlexBasis(
          iris::ui::maxOrDefined(resolvedFlexBasis, paddingAndBorder));
    }
    ...
    if ((!isMainAxisRow && node->style().overflow() == Overflow::Scroll) ||
      ...
      if (iris::ui::isUndefined(childWidth) && iris::ui::isDefined(width)) {
        childWidth = width;
    ...
    if ((isMainAxisRow && node->style().overflow() == Overflow::Scroll) ||
        node->style().overflow() != Overflow::Scroll) {
      if (iris::ui::isUndefined(childHeight) && iris::ui::isDefined(height)) {
        childHeight = height;
    ...
    const bool hasExactWidth =
        iris::ui::isDefined(width) && widthMode == SizingMode::StretchFit;
    ...
    const bool hasExactHeight =
        iris::ui::isDefined(height) && heightMode == SizingMode::StretchFit;
...
static void measureNodeWithMeasureFunc(
  ...
  const float innerWidth = iris::ui::isUndefined(availableWidth)
      ? availableWidth
      : iris::ui::maxOrDefined(0.0f, availableWidth - paddingAndBorderAxisRow);
  const float innerHeight = iris::ui::isUndefined(availableHeight)
      ? availableHeight
      : iris::ui::maxOrDefined(0.0f, availableHeight - paddingAndBorderAxisColumn);
...
inline bool isFixedSize(float dim, SizingMode sizingMode) {
  return sizingMode == SizingMode::StretchFit ||
      (iris::ui::isDefined(dim) && sizingMode == SizingMode::FitContent &&
       dim <= 0.0);
}
  ...
  if (isFixedSize(availableWidth, widthSizingMode) &&
    ...
    node->setLayoutMeasuredDimension(
        boundAxis(
            ...
            FlexDirection::Row,
            direction,
            iris::ui::isUndefined(availableWidth) ||
                    (widthSizingMode == SizingMode::FitContent &&
                     availableWidth < 0.0f)
                ? 0.0f
                : availableWidth,
    ...
    node->setLayoutMeasuredDimension(
        boundAxis(
            ...
            FlexDirection::Column,
            direction,
            iris::ui::isUndefined(availableHeight) ||
                    (heightSizingMode == SizingMode::FitContent &&
...
static void zeroOutLayoutRecursively(iris::ui::Node* const node) {
  node->getLayout() = {};
...
static void cleanupContentsNodesRecursively(iris::ui::Node* const node) {
...
static float calculateAvailableInnerDimension(
  ...
  if (iris::ui::isDefined(availableInnerDim)) {
    ...
    availableInnerDim = iris::ui::maxOrDefined(
        iris::ui::minOrDefined(availableInnerDim, maxInnerDim), minInnerDim);
  }
    ...
    for (auto child : children) {
      if (child->isNodeFlexible()) {
        if (singleFlexChild != nullptr ||
            iris::ui::inexactEquals(child->resolveFlexGrow(), 0.0f) ||
            iris::ui::inexactEquals(child->resolveFlexShrink(), 0.0f)) {
...
static float distributeFreeSpaceSecondPass(
  ...
  for (auto currentLineChild : flexLine.itemsInFlow) {
    ...
    if (iris::ui::isDefined(flexLine.layout.remainingFreeSpace) &&
      ...
      if (flexShrinkScaledFactor != 0) {
        ...
        if (iris::ui::isDefined(flexLine.layout.totalFlexShrinkScaledFactors) &&
        ...
        iris::ui::isDefined(flexLine.layout.remainingFreeSpace) &&
        flexLine.layout.remainingFreeSpace > 0) {
    ...
    if (childStyle.aspectRatio().isDefined()) {
      ...
      childCrossSizingMode = SizingMode::StretchFit;
                   ...
                   dimension(crossAxis), availableInnerCrossDim)) {
      childCrossSize = availableInnerCrossDim;
      childCrossSizingMode = iris::ui::isUndefined(childCrossSize)
      ...
      childCrossSizingMode =
          iris::ui::isUndefined(childCrossSize) || isLoosePercentageMeasurement
          ? SizingMode::MaxContent
...
static void distributeFreeSpaceFirstPass(
  ...
  for (auto currentLineChild : flexLine.itemsInFlow) {
    ...
    if (flexLine.layout.remainingFreeSpace < 0) {
      ...
      // Is this child able to shrink?
      if (iris::ui::isDefined(flexShrinkScaledFactor) &&
        ...
        boundMainSize = boundAxis(
            ...
            availableInnerMainDim,
            availableInnerWidth);
        if (iris::ui::isDefined(baseMainSize) && iris::ui::isDefined(boundMainSize) &&
      ...
      // Is this child able to grow?
      if (iris::ui::isDefined(flexGrowFactor) && flexGrowFactor != 0) {
        baseMainSize = childFlexBasis +
            flexLine.layout.remainingFreeSpace /
                flexLine.layout.totalFlexGrowFactors * flexGrowFactor;
        boundMainSize = boundAxis(
...
static void justifyMainAxis(
  ...
  if (sizingModeMainDim == SizingMode::FitContent &&
    ...
    if (style.minDimension(dimension(mainAxis)).isDefined() &&
      ...
      flexLine.layout.remainingFreeSpace = iris::ui::maxOrDefined(
    ...
    if (canSkipFlex) {
      ...
      if (isNodeBaselineLayout) {
        ...
        maxAscentForCurrentLine =
            iris::ui::maxOrDefined(maxAscentForCurrentLine, ascent);
        maxDescentForCurrentLine =
            iris::ui::maxOrDefined(maxDescentForCurrentLine, descent);
      } else {
        // The cross dimension is the max of the elements dimension since
        // there can only be one element in that cross dimension in the case
        // when the items are not baseline aligned
        flexLine.layout.crossDim = iris::ui::maxOrDefined(
...
static void calculateLayoutImpl(
  ...
  for (; startOfLineIterator != node->getLayoutChildren().end(); lineCount++) {
    ...
    if (sizingModeMainDim != SizingMode::StretchFit) {
      ...
      if (iris::ui::isDefined(minInnerMainDim) &&
          flexLine.sizeConsumed < minInnerMainDim) {
        availableInnerMainDim = minInnerMainDim;
      } else if (
          iris::ui::isDefined(maxInnerMainDim) &&
        ...
        if (!useLegacyStretchBehaviour &&
            ((iris::ui::isDefined(flexLine.layout.totalFlexGrowFactors) &&
              flexLine.layout.totalFlexGrowFactors == 0) ||
             (iris::ui::isDefined(node->resolveFlexGrow()) &&
    ...
    if (!sizeBasedOnContent && iris::ui::isDefined(availableInnerMainDim)) {
      flexLine.layout.remainingFreeSpace =
          availableInnerMainDim - flexLine.sizeConsumed;
      ...
      for (auto child : flexLine.itemsInFlow) {
        ...
        if (alignItem == Align::Stretch &&
          ...
          if (!child->hasDefiniteLength(
            ...
            const SizingMode childWidthSizingMode =
                iris::ui::isUndefined(childWidth) ||
                    (!isMainAxisRow && crossAxisDoesNotGrow)
                ? SizingMode::MaxContent
                : SizingMode::StretchFit;
            const SizingMode childHeightSizingMode =
                iris::ui::isUndefined(childHeight) ||
                    (isMainAxisRow && crossAxisDoesNotGrow)
                ? SizingMode::MaxContent
                : SizingMode::StretchFit;
          ...
          if (child->style().flexStartMarginIsAuto(crossAxis, direction) &&
              child->style().flexEndMarginIsAuto(crossAxis, direction)) {
            leadingCrossDim += iris::ui::maxOrDefined(0.0f, remainingCrossDim / 2);
          } else if (child->style().flexEndMarginIsAuto(crossAxis, direction)) {
            // No-Op
          } else if (child->style().flexStartMarginIsAuto(
                         crossAxis, direction)) {
            leadingCrossDim += iris::ui::maxOrDefined(0.0f, remainingCrossDim);
    ...
    maxLineMainDim =
        iris::ui::maxOrDefined(maxLineMainDim, flexLine.layout.mainDim);
  }
      ...
      for (; iterator != node->getLayoutChildren().end(); iterator++) {
        ...
        if (child->style().positionType() != PositionType::Absolute) {
          ...
          if (child->isLayoutDimensionDefined(crossAxis)) {
            lineHeight = iris::ui::maxOrDefined(
                lineHeight,
          ...
          switch (resolveChildAlignment(node, child)) {
            ...
            case Align::Stretch: {
              ...
              if (!child->hasDefiniteLength(
                ...
                if (!(iris::ui::inexactEquals(
                          childWidth,
                          child->getLayout().measuredDimension(
                              Dimension::Width)) &&
                      iris::ui::inexactEquals(
                          childHeight,
                          child->getLayout().measuredDimension(
                              Dimension::Height)))) {
                  calculateLayoutInternal(
...
void calculateLayout(
  ...
  if (node->hasDefiniteLength(Dimension::Width, ownerWidth)) {
    ...
    width = ownerWidth;
    widthSizingMode = iris::ui::isUndefined(width) ? SizingMode::MaxContent
                                               : SizingMode::StretchFit;
  }
  ...
  if (node->hasDefiniteLength(Dimension::Height, ownerHeight)) {
    ...
    height = ownerHeight;
    heightSizingMode = iris::ui::isUndefined(height) ? SizingMode::MaxContent
                                                 : SizingMode::StretchFit;
  }
