...
static inline void setFlexStartLayoutPosition(
    const iris::ui::Node* const parent,
    iris::ui::Node* child,
...
static void positionAbsoluteChild(
    const iris::ui::Node* const containingNode,
    const iris::ui::Node* const parent,
    iris::ui::Node* child,
    const Direction direction,
    const FlexDirection axis,
...
void layoutAbsoluteChild(
  ...
  if (iris::ui::isUndefined(childWidth) ^ iris::ui::isUndefined(childHeight)) {
    if (childStyle.aspectRatio().isDefined()) {
      if (iris::ui::isUndefined(childWidth)) {
        childWidth = marginRow +
            (childHeight - marginColumn) * childStyle.aspectRatio().unwrap();
      } else if (iris::ui::isUndefined(childHeight)) {
  ...
  if (iris::ui::isUndefined(childWidth) || iris::ui::isUndefined(childHeight)) {
    childWidthSizingMode = iris::ui::isUndefined(childWidth)
        ? SizingMode::MaxContent
        : SizingMode::StretchFit;
    childHeightSizingMode = iris::ui::isUndefined(childHeight)
        ? SizingMode::MaxContent
        : SizingMode::StretchFit;
    ...
    // browsers implement.
    if (!isMainAxisRow && iris::ui::isUndefined(childWidth) &&
        widthMode != SizingMode::MaxContent &&
        iris::ui::isDefined(containingBlockWidth) && containingBlockWidth > 0) {
...
bool layoutAbsoluteDescendants(
    iris::ui::Node* containingNode,
    iris::ui::Node* currentNode,
