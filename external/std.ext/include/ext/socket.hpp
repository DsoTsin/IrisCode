...
namespace std {
    namespace ext {
        ...
        extern std::shared_ptr<ip> query_host(const string& host);
        class socket {
        public:
#if _WIN32
            typedef size_t type;
            ...
            static socket* connect_to_host(string const& host_addr, int port);
        protected:
            type m_fd;
            socket(type fd);
        };
        ...
        public:
            ...
            ssl_socket* connect_to_host(const string& host, int port);
            ssl_socket* make();
        private:
