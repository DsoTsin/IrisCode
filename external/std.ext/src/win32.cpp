...
registry registry::local_machine(const char* path) {
  return registry(HKEY_LOCAL_MACHINE, path, (flags)(8 | 1));
}
...
void registry::list(vector<string>& subitems) {
  ...
  retCode = RegQueryInfoKeyA((HKEY)m_key,           // key handle
  ...
  if (cSubKeys) {
    for (i = 0; i < cSubKeys; i++) {
      ...
      retCode = RegEnumKeyExA((HKEY)m_key, i, achKey, &cbName, NULL, NULL, NULL, &ftLastWriteTime);
      if (retCode == ERROR_SUCCESS) {
        subitems.push_back(achKey);
      }
    }
  }
}
