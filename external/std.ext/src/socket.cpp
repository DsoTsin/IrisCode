...
namespace std {
    namespace ext {
        ...
        socket::type connect_host(string const& host_addr, int port) {
          char port_str[32] = {0};
          ...
          int rv = getaddrinfo(host_addr.c_str(), port_str, &hints, &serverinfo);
          char ipbuf[46] = {};
          for (p = serverinfo; p != NULL; p = p->ai_next) {
            auto fd = ::socket(p->ai_family, p->ai_socktype, p->ai_protocol);
            if (p->ai_family == AF_INET) {
              struct sockaddr_in* sa = (struct sockaddr_in*)(p->ai_addr);
              inet_ntop(AF_INET, &(sa->sin_addr), ipbuf, sizeof(ipbuf) - 1);
              int ret = ::connect(fd, (const sockaddr*)p->ai_addr, p->ai_addrlen);
              if (ret == -1) {
                continue;
              }
              return fd;
              ...
              inet_ntop(AF_INET6, &(sa6->sin6_addr), ipbuf, sizeof(ipbuf) - 1);
              printf("%s\n", ipbuf);
          ...
          return (socket::type)~0ull;
        }

        socket* socket::connect_to_host(string const& host_addr, int port) {
          socket::type sock = connect_host(host_addr, port);
          return (sock != (socket::type)~0ull) ? new socket(sock) : nullptr;
        }
        ...
        ssl_socket* ssl_socket_factory::connect_to_host(const string& host, int port) {
          socket::type sock = connect_host(host_addr, port);
          return (sock != (socket::type)~0ull) ? new ssl_socket(sock, m_ssl_ctx): nullptr;
        }
#endif
    }
}
